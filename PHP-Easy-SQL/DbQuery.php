...
class DbQuery extends DbConfig   
{
    ...
    {
        $queryWhere = self::where($where);
        return self::query("delete from $table $queryWhere[0]", $queryWhere[1] );
    }
    ...
    public static function insert($table, $valueToInsert)
    {
       ...
       return self::query( substr($firstQuery, 0,-2) . substr($secondQuery, 0,-2). ')' , $valueToInsert);
    }
    ...
    public static function update( $table, $valueToUpdate, $where)
    {
        ...
        return self::query( $query, array_merge($valueToUpdate, $queryWhere[1]));
